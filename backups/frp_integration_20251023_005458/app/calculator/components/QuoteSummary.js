...
export default function QuoteSummary({ bom, inputs }) {
  // Merge supports for compatibility with both old and new BOM structure
  const bomWithSupports = {
    ...bom,
    supports: [...(bom.internalSupport || []), ...(bom.externalSupport || []), ...(bom.supports || [])]
  };
      ...
      <div className="mt-6 pt-6 border-t border-blue-400 grid grid-cols-3 gap-4 text-center">
        <div>
          <div className="text-2xl font-bold">
            {bomWithSupports.summary.totalPanels}
          </div>
        ...
        <div>
          <div className="text-2xl font-bold">
          {bomWithSupports.base.length + bomWithSupports.walls.length + bomWithSupports.partition.length + bomWithSupports.roof.length + bomWithSupports.supports.length + bomWithSupports.accessories.length}
          </div>
          <div className="text-xs text-blue-200 uppercase tracking-wide">
            Line Items
          </div>
        ...
        <div>
          <div className="text-2xl font-bold">
            RM {bomWithSupports.summary.totalCost.toLocaleString()}
          </div>
          <div className="text-xs text-blue-200 uppercase tracking-wide">
            Estimated Cost
          </div>
