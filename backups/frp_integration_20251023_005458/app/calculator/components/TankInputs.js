...
export default function TankInputs({ inputs, setInputs }) {
  ...
  const handleMaterialChange = (material) => {
    if (material === 'FRP') {
      // FRP is always Metric Type 2 + defaults to MS1390 standard
      setInputs(prev => ({
        ...prev,
        material: 'FRP',
        panelType: 'm',
        panelTypeDetail: 2,
        buildStandard: 'MS1390'  // ← Default FRP standard
      }));
    } else {
      // Steel materials default to SANS standard
      setInputs(prev => ({
        ...prev,
        material,
        buildStandard: prev.material === 'FRP' ? 'BSI' : prev.buildStandard  // Reset if coming from FRP
      }));
    }
  };
  ...
  return (
    <div className="space-y-5">
     ...
     <div>
      ...
      {/* Build Standard Selector - Material Specific */}
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-2">
          Build Standard
        </label>
        <select
          value={inputs.buildStandard || (inputs.material === 'FRP' ? 'MS1390' : 'BSI')}
        ...
        >
          {inputs.material === 'FRP' ? (
            // FRP Standards
            <>
              <option value="MS1390">MS1390:2010 (SPAN Approved)</option>
              <option value="SS245">SS245:BSEN13280</option>
            </>
          ) : (
            // Steel Standards
            <>
        ...
        <p className="text-xs text-gray-500 mt-1">
          {inputs.material === 'FRP' && inputs.buildStandard === 'MS1390' && 'Malaysian Standard for FRP tanks (SPAN approved)'}
          {inputs.material === 'FRP' && inputs.buildStandard === 'SS245' && 'Singapore Standard EN13280 for FRP tanks'}
          {inputs.material !== 'FRP' && inputs.buildStandard === 'SANS' && 'Progressive thickness based on height'}
          {inputs.material !== 'FRP' && inputs.buildStandard === 'BSI' && '5mm for 1-3 panels, 6mm base for 4+ panels'}
          {inputs.material !== 'FRP' && inputs.buildStandard === 'LPCB' && '5mm for 1-3 panels, 6mm base for 4+ panels'}
        </p>
      </div>
