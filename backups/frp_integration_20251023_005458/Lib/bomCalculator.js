...
import { calculateFRPTank } from './frpCalculator';
...
export function calculateBOM(inputs) {
  ...
  const thickness = getThicknessByHeight(height, panelType, inputs.buildStandard || 'SANS', material);
  ...
  if (material === 'FRP') {
    ...
    // Use dedicated FRP calculator with full 772 SKU support
    const frpResult = calculateFRPTank(inputs);

    // Return FRP-specific BOM structure
    return {
      base: frpResult.base,
      walls: frpResult.walls,
      partition: frpResult.partition,
      roof: frpResult.roof,
      internalSupport: frpResult.structural, // Corner angles + tie rods
      externalSupport: [], // FRP typically doesn't use external I-beams
      accessories: frpResult.accessories,
      summary: frpResult.summary
    };
  }
  ...
  if (supportType === 'internal') {
    bom.internalSupport = calculateInternalSupport(
      lengthPanels,
      widthPanels,
      heightPanels,
      panelType,
    ...
    bom.externalSupport = calculateExternalSupport(
      lengthPanels,
      widthPanels,
      height,
      panelType,
  ...
  // Calculate totals
  const allItems = [...bom.base, ...bom.walls, ...bom.partition, ...bom.roof, ...bom.internalSupport, ...bom.externalSupport, ...bom.accessories];
  ...
  bom.summary.supportItems = bom.internalSupport.length + bom.externalSupport.length;
 ...
 */
export function getThicknessByHeight(heightMeters, panelType, buildStandard = 'BSI', material = 'SS316') {
  // ← Changed default from 'SANS' to 'BSI'
  const heightMM = heightMeters * 1000;
  const panelSize = panelType === 'm' ? 1000 : 1220;
  const heightPanels = Math.ceil(heightMM / panelSize);

  // FRP Standards - Use standard FRP thickness regardless of standard choice
  if (material === 'FRP') {
    // FRP tanks use standard thickness based on height, not build standard
    // Both MS1390:2010 and SS245:BSEN13280 follow similar thickness principles
    return {
      base: 6.0,      // FRP base panels (35% fiberglass content)
      wall: 6.0,      // FRP wall panels
      roof: 4.5,      // FRP roof panels (lighter)
      tiers: [
