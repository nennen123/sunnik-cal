...
/**
 * BOX QUANTITY CONFIGURATION
 * Defines how many pieces per box for different accessories
 * /**
 * BOX QUANTITY CONFIGURATION
 * Defines how many pieces per box for different materials
 */
const BOX_QUANTITIES = {
  'HDG': 520,         // HDG bolts: 520 per box (confirmed)
  'MS': 500,          // MS bolts: TBD - update when confirmed
  'ZP': 500,          // ZP bolts: TBD - update when confirmed
  'SS316': 100,       // SS316 bolts: TBD - update when confirmed
  'SS304': 100,       // SS304 bolts: TBD - update when confirmed
  'HDG-China': 520    // HDG China: TBD - update when confirmed
};

/**
 * Get box quantity for a given bolt material
 */
function getBoxQuantity(sku) {
  // Extract material from SKU (e.g., BN-M10-25-HDG -> HDG)
  for (const material in BOX_QUANTITIES) {
    if (sku.includes(material)) {
      return BOX_QUANTITIES[material];
    }
  }
  // Default fallback
  return 100;
...
const BOX_QUANTITIES = {
  // Bolts & Nuts
  'BN-HDG': 520,        // HDG bolts: 520 per box
  'BN-MS': 500,         // MS bolts: TBD - update when confirmed
  'BN-ZP': 500,         // ZP bolts: TBD - update when confirmed
  'BN-SS316': 100,      // SS316 bolts: TBD - update when confirmed
  'BN-SS304': 100,      // SS304 bolts: TBD - update when confirmed
  'BN-HDG-China': 520,  // HDG China bolts: TBD - update when confirmed

  // Add more accessories here as needed
  // 'ACCESSORY-CODE': quantity
};

/**
 * Get box quantity for a given SKU prefix
 */
function getBoxQuantity(sku) {
  // Extract the material code from SKU (e.g., BN-M10-25-HDG -> BN-HDG)
  for (const key in BOX_QUANTITIES) {
    if (sku.includes(key.replace('BN-', ''))) {
      return BOX_QUANTITIES[key];
    }
  }
  // Default fallback
  return 100;
...
export function calculateBOM(inputs) {
  ...
  if (material === 'FRP') {
    ...
    const frpResult = calculateFRPTank(lengthPanels, widthPanels, height, perimeter, panelType, supportType, iBeamSize, inputs.bnwMaterial);
    return frpResult;
  }
  ...
  bom.accessories = calculateBoltsAndNuts(
    ...
    panelType,
    material
    inputs.bnwMaterial || 'HDG'
  );

  // Generate SKU based on BNW material selection
// Generate SKU based on BNW material selection
const boltMaterial = bnwMaterial || 'HDG';
console.log('🔧 BNW Material selected:', bnwMaterial, '→ Using:', boltMaterial);

// Map to actual SKU codes in CSV
const boltSKUMap = {
  'HDG': 'BN300A017',      // Replace with actual HDG bolt SKU from CSV
  'MS': 'BN300A018',       // Replace with actual MS bolt SKU from CSV
  'ZP': 'BN300A019',       // Replace with actual ZP bolt SKU from CSV
  'SS316': 'BN300A020',    // Replace with actual SS316 bolt SKU from CSV
  'SS304': 'BN300A017',    // Replace with actual SS304 bolt SKU from CSV
  'HDG-China': 'BN300A021' // Replace with actual HDG-China bolt SKU from CSV
};

const boltSKU = boltSKUMap[boltMaterial] || 'BN300A017';
console.log('🔧 Generated SKU:', boltSKU);
 ...
 * Formula: Count edges per surface × bolts per edge
 */
function calculateBoltsAndNuts(lengthPanels, widthPanels, heightPanels, panelType, material, bnwMaterial) {
  const accessories = [];

  // Bolts per side based on material and panel type
  const boltsPerSide = getBoltsPerSide(material, panelType);

  // Helper function to count edges on a rectangular surface
  const countEdges = (rows, cols) => {
    return (rows + 1) * cols + rows * (cols + 1);
  };

  // BASE surface (lengthPanels × widthPanels)
  const baseEdges = countEdges(lengthPanels, widthPanels);
  const baseBolts = baseEdges * boltsPerSide;

  // ROOF surface (same as base)
  const roofBolts = baseBolts;

  // WALL surfaces
  // Length walls (2 walls): heightPanels × lengthPanels each
  const lengthWallEdges = countEdges(heightPanels, lengthPanels);
  const lengthWallBolts = lengthWallEdges * boltsPerSide * 2; // × 2 walls

  // Width walls (2 walls): heightPanels × widthPanels each
  const widthWallEdges = countEdges(heightPanels, widthPanels);
  const widthWallBolts = widthWallEdges * boltsPerSide * 2; // × 2 walls

  // Total bolts needed
  const totalBoltsNeeded = baseBolts + roofBolts + lengthWallBolts + widthWallBolts;

  // Generate SKU based on BNW material selection
  const boltMaterial = bnwMaterial || 'HDG';
  const boltSKU = `BN-M10-25-${boltMaterial}`;

  // Get box quantity for this material
  const boxQuantity = getBoxQuantity(boltSKU);

  // Calculate boxes needed (round up)
  const boxesNeeded = Math.ceil(totalBoltsNeeded / boxQuantity);
  const totalBoltsSupplied = boxesNeeded * boxQuantity;
  const extraBolts = totalBoltsSupplied - totalBoltsNeeded;

  accessories.push({
    sku: boltSKU,
    description: `Bolts & Nuts M10×25mm (${boltMaterial}) - ${boxesNeeded} boxes (${totalBoltsNeeded} pcs needed, ${totalBoltsSupplied} pcs supplied)`,
    quantity: boxesNeeded,  // Quantity is BOXES, not individual bolts
    unitPrice: 0,
    ...
    metadata: {
      ...
      piecesNeeded: totalBoltsNeeded,
      piecesSupplied: totalBoltsSupplied,
      extraPieces: extraBolts,
      wastePercentage: ((extraBolts / totalBoltsNeeded) * 100).toFixed(1)
    }
  });
...
/**
 * Get bolts per side based on material and panel type
 */
...
/**
 * Get bolt SKU based on material
 */
...
/**
 * Calculate Internal Support (Tie Rods/Stays)
...
function calculateInternalSupport(lengthPanels, widthPanels, heightPanels, panelType, materialCode, partitionCount = 0) {
  ...
  // Calculate stay quantity per direction
  const lengthStays = widthPanels - 1; // Runs across width
  const widthStays = lengthPanels - 1; // Runs across length

  // Calculate lengths (spans across tank)
  const lengthMeters = lengthPanels * panelSize;
  const widthMeters = widthPanels * panelSize;

  // Bottom tier - Ø18mm (high pressure)
  if (heightPanels >= 1) {
    // Length direction stays
    if (lengthStays > 0) {
      supports.push({
  ...
  // Middle/Top tiers - Ø14mm (lower pressure)
  if (heightPanels >= 2) {
  ...
  if (partitionCount > 0) {
    ...
    // Upper tier partition stays - Ø14mm
 ...
 * FRP Tank calculation
 */
function calculateFRPTank(lengthPanels, widthPanels, height, perimeter, panelType, supportType, iBeamSize, bnwMaterial) {
