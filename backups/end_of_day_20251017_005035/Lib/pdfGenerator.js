...
export async function generatePDF(bom, inputs) {
  ...
  // Add Logo with automatic aspect ratio detection
  try {
    const loadImage = () => {
    ...
    try {
      ...
      // Calculate proper aspect ratio from actual image
      const imgWidth = logoImg.width;
      const imgHeight = logoImg.height;
      const aspectRatio = imgWidth / imgHeight;

      // Set desired height and calculate width based on actual ratio
      const logoHeight = 15;
      const logoWidth = logoHeight * aspectRatio;

      console.log(`Logo dimensions: ${imgWidth}×${imgHeight}, Aspect ratio: ${aspectRatio.toFixed(2)}`);
      ...
      doc.text('SUNNIK', margin, yPosition + 7);
    }
  } catch (error) {
  ...
  doc.text('Water Tank Solutions', margin, yPosition + 14);
