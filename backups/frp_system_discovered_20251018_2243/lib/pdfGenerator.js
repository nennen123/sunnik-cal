...
export async function generatePDF(bom, inputs) {
  ...
  const specs = [
    ...
    { label: 'Build Standard:', value: getBuildStandardName(inputs.buildStandard, inputs.material) },
...
function getBuildStandardName(standard, material) {
  // FRP Standards
  if (material === 'FRP') {
    const frpStandards = {
      'MS1390': 'MS1390:2010 (SPAN Approved)',
      'SS245': 'SS245:BSEN13280 (Singapore Standard)'
    };
    return frpStandards[standard] || standard || 'MS1390:2010';
  }

  // Steel Standards
  const steelStandards = {
    'SANS': 'SANS 10329:2020 (South African)',
    'BSI': 'BSI (British Standard)',
    'LPCB': 'LPCB (Loss Prevention Certification Board)'
  };
  return steelStandards[standard] || standard || 'SANS 10329:2020';
}
