...
6. **Bolts:** purchased from suppliers per BOX; quoted to customers as aggregate **BNW SET** lines in lots of 100 (`qty = ceil(pieces/100)`), priced via `market_final_price` of `BNW-{SS316|SS304|HDG|MS}-SET` (steel) and `BNW-FRP-SET` (FRP). **NEVER** use `conversion_rate` or `PURCHASE_UOM` in quoting — those are procurement-side fields. True per-piece `BN300` itemization is not implemented (see Parked in `BUILD_LIST.md`). ⚠️ Known gap: the FRP path emits a nonexistent `BNW-{mat}-FRP` SKU that fuzzy-resolves to the correct `BNW-FRP-SET` price — a phantom SKU shows on the BOM/PDF but the price is right (see BUILD_LIST P2).
