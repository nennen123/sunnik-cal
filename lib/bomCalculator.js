...
function generateSKU(panelType, location, thickness, size, material) {
  ...
  // 2.5 → "25", 3.0 → "3", 4.5 → "45", 5.0 → "5"
  const thicknessCode = (thickness % 1 === 0)
    ? Math.round(thickness).toString()  // Whole number: 3.0 → "3", 5.0 → "5"
    : thickness.toString().replace('.', '');  // Decimal: 4.5 → "45", 2.5 → "25"
